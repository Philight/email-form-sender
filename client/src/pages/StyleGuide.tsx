      ...
      <table>
        ...
        <tbody>
          <tr>
            ...
            {Object.keys(TAILWIND_FONT_SIZES).map(col => (
              <td className={[].css()}>{TAILWIND_FONT_SIZES[col]['font-size']}</td>
            ))}
          </tr>
          <tr>
            <td className={[].css()}>line-height</td>
            {Object.keys(TAILWIND_FONT_SIZES).map(col => (
              <td className={[].css()}>{TAILWIND_FONT_SIZES[col]['line-height']}</td>
            ))}            
          </tr>          
        </tbody>
      </table>
...
const StyleGuide = (props: PageProps) => {
  const { className } = props;

  return (
    <main className={['style-guide__c page', className].css()}>
      <h1>Typography</h1>
      <FontSizes />
    </main>
  );
